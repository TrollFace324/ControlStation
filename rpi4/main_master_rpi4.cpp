...
static constexpr int SECOND_RESPONSE_TIMEOUT_MS = 5000;
        ...
        while (waitingResponse_) {
            ...
            if (serial.readLine(line, 20)) {
                ...
                if (!needsRepeatedResponse) {
                    ...
                    waitingResponse_ = false;
                    collectedResponses.push_back(parsedResponse->raw);
                    gpio.toggleLed();
                ...
                if (!parsedResponse->state.has_value() || *parsedResponse->state != expectedState) {
                    waitingResponse_ = false;
            ...
            if (waitMs >= timeoutMs) {
                ...
                return {false, 504, {}, {}, "Slave response timeout", false};
            }
        }

        return {false, 500, {}, {}, "Request processing stopped unexpectedly", false};
    }

    RequestResult requestStatus(SerialPort& serial, GpioOutputs& gpio, const std::string& requestId) {
        if (!waitingSecondResponse_) {
            return {false, 404, {}, {}, "No pending second slave response", false};
        }
        ...
        while (true) {
            ...
            if (serial.readLine(line, 20)) {
                ...
                if (!parsedResponse->state.has_value() || *parsedResponse->state != 1) {
                ...
                waitingSecondResponse_ = false;
                pendingSecondRequestId_.clear();
                pendingSecondResponses_.push_back(parsedResponse->raw);
                gpio.toggleLed();
                std::cout << "Received second repeated response: " << line << std::endl;
