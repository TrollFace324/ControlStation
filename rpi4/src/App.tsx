...
const batteryPercent = 38;
const batteryRadius = 46;
const lapaGuideTravel = 272;
const autoRequestId = "-1";
const autoRequestIdleMs = 10_000;
const autoRequestCheckIntervalMs = 1_000;
...
function App() {
  ...
  const [kranAction, setKranAction] = useState<KranAction>("throw");
  ...
  const lastRequestStartedAtRef = useRef(Date.now());
  const sendSlaveRequestRef = useRef<
    | ((requestId: string, options?: SendSlaveRequestOptions) => Promise<void>)
    | null
  >(null);
  ...
  }, [isAutonomyPositionMenuOpen, isServoMenuOpen, isTelemetryMenuOpen]);
  ...
  const awaitSecondSlaveResponse = async (requestId: string) => {
    while (true) {
      await wait(250);
      ...
      if (!response.ok || !payload?.ok) {
        ...
        setRequestMessage(payload?.error ?? `HTTP ${response.status}`);
  ...
  const sendSlaveRequest = async (
    ...
    const { isAutoRequest = false } = options;
    ...
    try {
      ...
      if (!response.ok || !payload?.ok) {
        if (isAutoRequest) {
          setRequestState("error");
          setRequestMessage(
            `Auto request REQ,${requestId} failed: ${payload?.error ?? `HTTP ${response.status}`}`,
          );
          setLastRequestId(requestId);
          setLastSlaveResponse(slaveResponse);
        } else {
      ...
      if (payload?.awaitingSecondResponse) {
        ...
        setSecondResponseOverlayMessage("Ожидание второго ответа от slave...");
        await awaitSecondSlaveResponse(requestId);
        return;
      }
      ...
      if (isAutoRequest) {
        ...
        setRequestMessage(
          error instanceof Error
            ? `Auto request REQ,${requestId} unavailable: ${error.message}`
            : `Auto request REQ,${requestId} unavailable`,
        );
        setLastRequestId(requestId);
        setLastSlaveResponse("");
      } else {
  ...
  useEffect(() => {
    const intervalId = window.setInterval(() => {
      ...
      if (Date.now() - lastRequestStartedAtRef.current < autoRequestIdleMs) {
        return;
      }

      void sendSlaveRequestRef.current?.(autoRequestId, {
        isAutoRequest: true,
      });
    }, autoRequestCheckIntervalMs);
            ...
            >
              ...
              <ul className="status-card__list">
                {systemStatuses.map((statusItem) => (
                  <li key={statusItem} className="status-card__item">
          ...
          <div className="kran-controls">
            ...
            >
              ...
              <div className="kran-mode-card__selector">
                ...
                >
                  <button
                    type="button"
                    className={`kran-mode-card__arrow ${requestClass(requestIds.kranPositionUp)}`.trim()}
                    data-request-id={requestIds.kranPositionUp}
                    onClick={() => {
                      handleStaticRequestClick(requestIds.kranPositionUp);
                    }}
                  >
                    ⌃
                  </button>
                  ...
                  <button
                    type="button"
                    className={`kran-mode-card__arrow ${requestClass(requestIds.kranPositionDown)}`.trim()}
                    data-request-id={requestIds.kranPositionDown}
                    onClick={() => {
                      handleStaticRequestClick(requestIds.kranPositionDown);
                    }}
                  >
                    ⌄
                  </button>
            ...
            >
              <button
                ...
                data-request-id={requestIds.kranHome}
                aria-pressed={kranAction === "home"}
                onClick={() => {
                  handleStaticRequestClick(requestIds.kranHome, () => {
                    setKranAction("home");
                  });
                }}
              >
              ...
              <button
                ...
                data-request-id={requestIds.kranThrow}
                aria-pressed={kranAction === "throw"}
                onClick={() => {
                  handleStaticRequestClick(requestIds.kranThrow, () => {
                    setKranAction("throw");
                  });
                }}
              >
                ...
                >
                  {telemetryOptions.map((option) => (
                    <button
                      key={option.key}
                      type="button"
                      className={`telemetry-dropdown__option ${requestClass(option.requestId)} ${selectedTelemetryKey === option.key ? "is-selected" : ""}`.trim()}
